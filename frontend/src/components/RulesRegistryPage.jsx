  ...
  if (!rule) {
    return (
      <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-6 text-center">
        ...
        <p className="text-sm text-gray-500">Select a rule to inspect formula, source, and enforcement details.</p>
      </div>
    )
  }
    ...
    <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] overflow-hidden">
      <div className="border-b border-white/[0.06] px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-1 leading-relaxed">{summarizeRule(rule)}</p>
          </div>
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-gray-600">Category</p>
            <p className="text-gray-300 mt-1">{labelize(rule.category)}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-gray-600">Stage</p>
            <p className="text-gray-300 mt-1">{labelize(rule.stage)}</p>
          </div>
          <div>
        ...
        <DetailBlock label="Conditions" value={conditions} />
        <DetailBlock label="Actions" value={actions} />
...
function RuleEditorModal({ mode, rule, onClose, onSave }) {
  ...
  const title = mode === 'create' ? 'Teach Rule' : `Edit v${Number(rule?.version || 1) + 1}`
        ...
        <div className="flex items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {mode === 'create' ? 'Speak or type the lesson, then review the draft.' : 'Edits save as the next version.'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
          ...
          <label className="space-y-1.5">
            ...
            <select
              ...
              className="w-full rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-300/40"
            >
              {FALLBACK_STATUSES.map((item) => <option key={item} value={item}>{labelize(item)}</option>)}
            </select>
          </label>
          <label className="space-y-1.5">
          ...
          <label className="space-y-1.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Conditions JSON</span>
            <textarea
          ...
          <label className="space-y-1.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500">Actions JSON</span>
            <textarea
    ...
    <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h2 className="text-sm font-bold text-white">Rules/Audit Harness</h2>
        </div>
        <span className="text-[10px] text-gray-600 uppercase tracking-wider">Backend optional</span>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2">
        <input
          value={jobId}
          onChange={(event) => setJobId(event.target.value)}
          placeholder="Job ID or slug"
      ...
      {result && (
        <div className="mt-3 rounded-lg border border-white/[0.06] bg-black/20 px-3 py-3">
          <div className="grid gap-2 sm:grid-cols-4">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-600">Field</p>
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-600">Run</p>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-white">Ruleset Version</h2>
          </div>
          <p className="mt-1 text-xs text-gray-500">
            {current ? `Current ruleset v${current.version} tracks ${current.rule_count} rules, ${current.active_count} active.` : 'No ruleset snapshot yet.'}
          </p>
        </div>
        <button
          type="button"
        ...
        {rulesets.slice(0, 6).map((item, index) => (
          <div key={item.version} className="rounded-md border border-white/[0.06] bg-white/[0.025] px-3 py-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-bold text-white">Ruleset v{item.version}</span>
              <span className="text-[10px] uppercase tracking-wider text-gray-600">{labelize(item.change_type)}</span>
            </div>
...
export default function RulesRegistryPage() {
  ...
  const rollbackRuleset = async (ruleset) => {
    ...
    if (!window.confirm(`Roll the entire rules registry back to ruleset v${ruleset.version}? This creates a new current ruleset version.`)) return
    try {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-6 sm:py-10 space-y-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Manage estimating rules and inspect their saved versions.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <SelectFilter icon={Filter} label="category" value={category} values={categoryOptions} onChange={setCategory} />
          <SelectFilter icon={SlidersHorizontal} label="stage" value={stage} values={stageOptions} onChange={setStage} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_400px] gap-4">
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] overflow-x-auto">
            <div className="grid min-w-[720px] grid-cols-[1fr_72px_96px_104px_96px] gap-3 border-b border-white/[0.06] px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-600">
              <span>Rule</span>
              <span>Version</span>
              <span>Status</span>
              <span>Category</span>
                ...
                {filteredRules.map((rule) => {
                  ...
                  return (
                    <button
                      ...
                      className={`grid w-full min-w-[720px] grid-cols-[1fr_72px_96px_104px_96px] gap-3 px-4 py-3 text-left transition-colors ${
                    ...
                    >
                      <span className="min-w-0">
                        ...
                        <span className="block truncate text-xs text-gray-500 mt-0.5">{summarizeRule(rule)}</span>
                      </span>
                      <span className="self-start text-xs font-bold text-gray-300">v{rule.version || 1}</span>
                      <span className={`self-start justify-self-start text-[10px] font-bold px-2 py-0.5 rounded-md border uppercase tracking-wider ${statusClass(rule.status)}`}>
                        {rule.status || 'unknown'}
                      </span>
                      <span className="self-start truncate text-xs text-gray-400">{labelize(rule.category)}</span>
                      <span className="self-start truncate text-xs text-gray-500">{labelize(rule.stage)}</span>
                    </button>
                  )
                })}
              </div>
